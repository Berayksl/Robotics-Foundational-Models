##manipulates the logits based on the Q-values (just adds the raw Q values) (2/2/2026)
        ...
        with torch.no_grad():
            for eps_idx in range(task.max_steps):
                ...
                if np.linalg.norm(current_state[:2] - np.array(goals[0]['center'])) <= goals[0]['radius'] and not target_reached:
                    target_reached = True
                    print("Task satisifed at step", eps_idx)
                ...
                if action != "sub_done" and not target_reached:
                    ...
                    for a in action_list:
                        if a not in regular_actions:
                            #Q_values[action_list.index(a)] = -float('inf')
                            continue
                        else:
                            ...
                            q_vals = Q_net(x).squeeze().cpu().numpy()  # (n_actions,)
                            Q_values[action_list.index(a)] = q_vals[idx_to_action.index(a)]

                    alpha = 0.5  # a temperature parameter that adjusts the sharpness of the bias
                    robustness_weights = Q_values
                    print(f"time step: {eps_idx}")
                    print("Robustness weights:", robustness_weights)

                    #set the elements with -inf robustness to -inf weight
                    #robustness_weights[np.isneginf(robustness_values)] = -float('inf')

                    beta = 1 # scaling factor for robustness influence (best value = 100)
...
if __name__ == "__main__":
    ...
    task = {'sample_id': 'task=ObjectNavType,house=30,sub_house_id=2', 'house_id': '30', 'task_type': 'ObjectNavType', 'sub_house_id': 30, 'needs_video': True, 'raw_navigation_camera': '', 'sensors_path': '', 
            'observations': {'goal': 'go to a bowl', 'initial_agent_location': np.array([3.5,  0.90099216,  5 , 90. ,0.]), 'actions': [], 'time_ids': [], 
    ...
    config = {
        'house_index': 30,
        'init_loc':[3.5, 5, 90], #initial location of the agent (x, y)
        "dt": 1,
        "render": False,
		'dt_render': 0.01,
