#manipulates the logits based on the robustness values over a horizon (12/21/2025)
...
def parse_args():
    ...
    parser.add_argument("--training_run_id", default='SigLIP-ViTb-3-CHORESNav-L' ,type=str)
        ...
        with torch.no_grad():
            for eps_idx in range(task.max_steps):
                ...
                if action != "sub_done" and not target_reached:
                    regular_actions = ['m', 'r', 'l', 'b', 'ls', 'rs'] #actions that move the robot
                    robustness_values = np.full(len(action_list), -float("inf"))
                    ...
                    beta = 100  # scaling factor for robustness influence
                ...
                #print("goal_reached:", target_reached)
...
def mpc_terminal_max_robustness_exact(
    x,
    depth,
    horizon,
    regular_actions,
    ...
    """
    Returns max terminal robustness reachable from state x
    in exactly (horizon - depth) steps.
    """
    if depth == horizon:
        return calculate_robustness(x, goal_center=goal_center)

    best_rho = -float("inf")

    for a in regular_actions:
        x_next, _, _ = unicycle_step(x, a)
        rho = mpc_terminal_max_robustness_exact(
            x_next,
            depth + 1,
            horizon,
            regular_actions,
...
def mpc_Q_rho_exact(
    ...
    regular_actions,
    goal_center,
):  #wrapper for fixing the first acion of mpc_terminal_max_robustness_exact
    x1, _, _ = unicycle_step(x0, first_action)

    return mpc_terminal_max_robustness_exact(
        x=x1,
        depth=1,
        horizon=horizon,
        regular_actions=regular_actions,
...
def mpc_terminal_max_robustness_fast(
    x,
    depth,
    horizon,
    regular_actions,
    ...
    """
    Exact MPC with memoization and branch-and-bound pruning.
    """
    ...
    for a in regular_actions:
        ...
        # Upper bound pruning
        if rho_max <= best_so_far:
            continue

        rho = mpc_terminal_max_robustness_fast(
            x_next,
            depth + 1,
            horizon,
            regular_actions,
            goal_center,
            cache,
            max(best_so_far, best_rho),
            rho_max,
        )

        best_rho = max(best_rho, rho)

        # Early exit if max possible robustness reached
        if best_rho >= rho_max:
            break
...
def mpc_Q_rho_exact_fast(
    ...
    cache = {}
    rho_max = 0.5  # adjust to your robustness definition

    return mpc_terminal_max_robustness_fast(
        x=x1,
        depth=1,
        horizon=horizon,
        regular_actions=regular_actions,
