...
def kl_project_action_distribution_with_robustness_v3(
    *,
    ...
    stl_task_str: str,
    goals: Dict[int, Dict],               # goals[goal_id] has {'center':..., 'radius':...}
    obstacles: Dict[int, Dict],             # obstacles[obstacle_id] has {'center':..., 'half_extents':...}
    forward_propagate,                    # (env, current_state, current_t, action, horizon, q_net) -> future_trajectory
    calculate_goal_predicate_robustness,       # (trajectory, center, radius) -> (T,) array
    calculate_obstacle_predicate_robustness, # (trajectory, obstacle_center, obstacle_half_extents) -> (T,) array
    get_task_robustness,                 # (stl_task_str, predicate_signals) -> robustness
    ...
    for i, a in enumerate(regular_actions):
        ...
        assert len(temp_trajectory) == horizon + 1, (current_t, len(state_trajectory), len(future_trajectory), len(temp_trajectory))

        num_goals = len(goals)
        num_obstacles = len(obstacles)
        predicate_signals = np.ones((len(temp_trajectory), num_goals + num_obstacles), dtype=np.float64) * -999.0
        for goal_id in goals.keys():
            goal = goals[goal_id]
            pred_rob = calculate_goal_predicate_robustness(temp_trajectory, goal['center'], goal['radius'])
            predicate_signals[:, goal_id] = pred_rob

        for obstacle_id in obstacles.keys():
            obstacle = obstacles[obstacle_id]
            pred_rob = calculate_obstacle_predicate_robustness(env, temp_trajectory, obstacle['center'], obstacle['half_extents'])
            predicate_signals[:, num_goals + obstacle_id] = pred_rob
    ...
    if tau == 1.0:
        ...
        if mass <= 0:
            ...
            #return argmax(q_vals), but if all g_vals are negative, this will return the least negative (closest to 0)
            best = int(np.argmax(g_vals))
            q[best] = 1.0
            return None, g_vals
        q[safe] = p[safe] / mass
    ...
    # ---- 3) Feasibility check---- for tau != 1 cases
    if not np.any(v > 0.5):
        print("Infeasible: no action can satisfy STL under π_phi rollout.")
        return None, g_vals
