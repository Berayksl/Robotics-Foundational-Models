##manipulates the logits based on the Q-values (adds normalized values) (2/2/2026)
        ...
        with torch.no_grad():
            while eps_idx < task.max_steps:
                ...
                if action != "sub_done" and not target_reached:
                    ...
                    Q_values = Q_net(x).squeeze().cpu().numpy()  # (n_actions,)
                    ...
                    A = Q_values - Q_values.mean(axis=-1, keepdims=True) #advantage values
                    A = A / (Q_values.std(axis=-1, keepdims=True) + 1e-6)   # per-state normalization

                    #print("Normalized Q values:", A)

                    # normalize logits so both are comparable
                    L = regular_action_logits - regular_action_logits.mean(axis=-1, keepdims=True)
                    L = L / (regular_action_logits.std(axis=-1, keepdims=True) + 1e-6)
                    ...
                    # A = (Q - Q_min) / (Q_max - Q_min + eps)  # mapped to [0,1]

                    # # ---------- Logits → [0, 1] ----------
                    # L_raw = regular_action_logits  # shape (..., 6)

                    # L_min = L_raw.min(axis=-1, keepdims=True)
                    # L_max = L_raw.max(axis=-1, keepdims=True)

                    # L = (L_raw - L_min) / (L_max - L_min + eps)  # mapped to [0,1]

                    print("Normalized Q values:", A)
                    print("Normalized logit values:", L)

                    normalized_q_list.append(A)
                    normalized_logit_list.append(L)
                    regular_Q_vals.append(Q_values)
                    regular_logits.append(regular_action_logits)

                    # print("Normalized Logits:", L)
                    alpha_default = 1
                    #beta = 1.0
                    beta = beta_exp_saturating(eps_idx, STL_horizon, beta_min= 0.1, beta_max = 2.0, k = -3)
                    alpha = 0 if main_task_done else alpha_default

                    print(f"alpha: {alpha}, beta: {beta}")

                    if alpha == 0:
                        action_idx = np.argmax(Q_values)  # (1,)
                        action = regular_actions[action_idx]

                    else:
                        #modified_logits = L + beta * A
                        #modified_logits = alpha * regular_action_logits + beta * Q_values
                        modified_logits = alpha * L + beta * A
    
                        modified_logits = torch.tensor(modified_logits, dtype=torch.float32, device=device)
                ...
                #print("Task success so far:", success)
...
if __name__ == "__main__":
    ...
    task = {'sample_id': 'task=ObjectNavType,house=30,sub_house_id=2', 'house_id': '30', 'task_type': 'ObjectNavType', 'sub_house_id': 30, 'needs_video': True, 'raw_navigation_camera': '', 'sensors_path': '', 
            'observations': {'goal': 'go to a bowl', 'initial_agent_location': np.array([1.5,  0.90099216,  3 , 90. ,0.]), 'actions': [], 'time_ids': [], 
    ...
    config = {
        'house_index': 30,
        'init_loc':[1.5, 3.0, 90], #initial location of the agent (x, y)
        "dt": 1,
        "render": False,
		'dt_render': 0.01,
