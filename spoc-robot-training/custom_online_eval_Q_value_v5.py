##Solving KL divergence  optimization problem (2/17/2026) CASE-1
...
from KL_divergence import kl_project_action_distribution_with_robustness, kl_project_action_distribution_with_robustness_v2, kl_project_action_distribution_with_robustness_v3
            ...
            while eps_idx < task.max_steps:
                ...
                if not stl_task_done: #modify the distribution if STL task is not yet satisfied
                    ...
                    if not main_task_done:
                        #solve the optimization problem to get the new action distribution (q):
                        # q, g_vals = kl_project_action_distribution_with_robustness(
                        #     env = env_2d,
                        ...
                        #     calculate_predicate_robustness = calculate_predicate_robustness,
                        #     get_task_robustness = get_task_robustness,
                        #     state_trajectory = state_trajectory,
                        #     p_probs = p,
                        #     robustness_threshold=0.2, #we want to ensure that the chosen action distribution leads to a trajectory with non-negative robustness (i.e., satisfying STL constraints)
                        # )

                        # q, g_vals = kl_project_action_distribution_with_robustness_v2(
                        #     env = env_2d,
                        #     current_state = np.array(current_state),
                        #     current_t = eps_idx,
                        #     horizon = STL_horizon,
                        ...
                        #     state_trajectory = state_trajectory,
                        #     p_probs = p,
                        #     violation_epsilon=0.0, #we want to ensure that the chosen action distribution leads to a trajectory with non-negative robustness (i.e., satisfying STL constraints)
                        # )
                        ...
                        q, g_vals = kl_project_action_distribution_with_robustness_v3(
                            ...
                            goals = goals,
                            forward_propagate = forward_propagate,
                            calculate_predicate_robustness = calculate_goal_predicate_robustness,
                            get_task_robustness = get_task_robustness,
                            state_trajectory = state_trajectory,
                            p_probs = p,
                ...
                # #check whether there is a mismatch between the predicted dynamics and the actual dynamics in THOR after taking the action, which could indicate a sim-to-real gap in dynamics and help explain some failure cases
                # if action in regular_actions:
                #     #check if the next state is same as 2D environment's dynamics prediction (for debugging the sim-to-real gap in dynamics)
                #     full_pose = task.controller.get_current_agent_full_pose() #(x, y, z) *y is height
                #     THOR_next_state = (full_pose['position']['x'], full_pose['position']['z'], full_pose['rotation']['y']) #(x, y, theta in degrees)

                #     predicted_next_state = fast_step_discrete_with_collision(env_2d, current_state, action)[0]

                #     # print(f"THOR next state: {THOR_next_state}")
                #     # print(f"Predicted next state: {predicted_next_state}")

                #     ok, info = states_almost_same(THOR_next_state, predicted_next_state, pos_tol=1e-1, yaw_tol=1e-1)
                #     if not ok:
                #         # mark episode as "mismatch" and skip counting it
                #         print("MISMATCH:", info, "thor:", THOR_next_state, "pred:", predicted_next_state)
                #         mismatch_episode = True
                #         break
...
# def forward_propagate(env, past_trajectory, initial_state, current_t, first_action, horizon, stl_task_str, q_net):
#     regular_actions = ['m', 'b', 'l', 'r', 'ls', 'rs']
...
#         temp_trajectory = past_trajectory + state_trajectory #combine the past trajectory and the forward-propagated trajectory to calculate the STL robustness for the current trajectory

#         num_goals = len(goals)
#         predicate_signals = np.ones((len(temp_trajectory), num_goals), dtype=np.float64) * -999.0
#         for goal_id in goals.keys():
#             goal = goals[goal_id]
#             pred_rob = calculate_predicate_robustness(temp_trajectory, goal['center'], goal['radius'])
#             predicate_signals[:, goal_id] = pred_rob

#         robustness = get_task_robustness(stl_task_str, predicate_signals)

#         if robustness > 0:
#             break #if the STL constraints are already satisfied, we can stop the forward propagation early
...
if __name__ == "__main__":
    ...
    # task = {'sample_id': 'task=ObjectNavType,house=8,sub_house_id=9', 'house_id': '9', 'task_type': 'ObjectNavType', 'sub_house_id': 9, 'needs_video': False, 'raw_navigation_camera': '', 'sensors_path': '', 
    #         'observations': {'goal': 'go to a bowl', 'initial_agent_location': np.array([1,  0.90099216,  6.5 , 90. ,0.]), 'actions': [], 'time_ids': [], 
    #         'templated_task_type': '{"task_type": "ObjectNavType", "house_index": 9, "agent_starting_position": [1, 0.9009921550750732, 6.5], "agent_y_rotation": 90.0, "expert_length_bucket": "short", "expert_length": 30, "broad_synset_to_object_ids": {"bowl.n.03": ["Bowl|3|19"]}, "synset_to_object_ids": {"bowl.n.03": ["Bowl|3|19"]}, "synsets": ["bowl.n.03"], "extras": {"chosen_object_id": "Bowl|3|19"}, "natural_language_spec": "go to a bowl", "task_path": "/net/nfs.cirrascale/prior/datasets/vida_datasets/object_nav_v3_benchmark/ObjectNavType/val/013653/raw_navigation_camera__0.mp4", "hypernyms": ["instrument.n.01"], "freqs": [15]}'}}
