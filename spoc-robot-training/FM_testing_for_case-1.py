#for recording video of Case-1 SPOC Path
                ...
                for eps_idx in range(task.max_steps):
                    ...
                    if display_topdown_realtime and hasattr(ev, "third_party_camera_frames"):
                        ...
                        if frames is not None and len(frames) > 0:
                            ...
                            topdown_rgb = topdown_rgb[135:h-135, 420:w-420] #crop to remove the background

                            topdown_bgr_native = cv2.cvtColor(topdown_rgb, cv2.COLOR_RGB2BGR)
                            topdown_disp = topdown_bgr_native

                            # # Display: optionally scale down/up for screen, but keep saving native
                            # topdown_disp = topdown_bgr_native
                            # if topdown_scale != 1:
                            #     th, tw = topdown_bgr_native.shape[:2]
                            #     topdown_disp = cv2.resize(
                            #         topdown_bgr_native,
                            #         (int(tw * topdown_scale), int(th * topdown_scale)),
                            #         interpolation=cv2.INTER_LINEAR,
                            #     )
                            ...
                            cv2.imshow("TopDown [VIS]", topdown_disp)
