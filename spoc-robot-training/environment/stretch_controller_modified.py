...
class StretchController:
    ...
    def _downscale_to_model_size(self, frame: np.ndarray) -> np.ndarray:
        """
        Convert whatever THOR rendered (e.g., 1920x1080) into the canonical
        model input size (224x396) while keeping aspect consistent.
        """
        INTEL_CAMERA_WIDTH  = 396
        INTEL_CAMERA_HEIGHT = 224

        # Ensure uint8 HxWxC
        if frame is None:
            return frame

        # If frame is already at target size, do nothing
        if frame.shape[0] == INTEL_CAMERA_HEIGHT and frame.shape[1] == INTEL_CAMERA_WIDTH:
            return frame

        # Downsample to exactly (396, 224)
        # INTER_AREA is best for downsampling
        return cv2.resize(frame, (INTEL_CAMERA_WIDTH, INTEL_CAMERA_HEIGHT), interpolation=cv2.INTER_AREA)
    

    def _crop_like_spoc(self, frame_224x396: np.ndarray) -> np.ndarray:
        """
        Apply the same 6px-per-side crop (for 396-wide reference),
        yielding 384x224 which your sensors expect.
        """
        if frame_224x396 is None:
            return frame_224x396

        # For canonical width=396, cutoff should be exactly 6
        cutoff = round(frame_224x396.shape[1] * 6 / 396)  # will be 6
        return frame_224x396[:, cutoff:-cutoff, :]

    @property
    def navigation_camera(self):
        frame_hr = self.controller.last_event.frame  # could be 1080p now
        frame = self._downscale_to_model_size(frame_hr)      # -> 224x396
        frame = self._crop_like_spoc(frame)                  # -> 224x384
        return frame

    @property
    def manipulation_camera(self):
        frame_hr = self.controller.last_event.third_party_camera_frames[0]  # could be 1080p now
        frame_hr = frame_hr[:, :, :3]  # ensure RGB only
        frame = self._downscale_to_model_size(frame_hr)      # -> 224x396
        frame = self._crop_like_spoc(frame)                  # -> 224x384
        return frame

    # ----------------------------
    # HIGH-RES CAMERAS FOR SAVING / DISPLAY (no downscale)
    # ----------------------------
    @property
    def navigation_camera_hr(self):
        # raw render resolution
        return self.controller.last_event.frame

    @property
    def manipulation_camera_hr(self):
        frame = self.controller.last_event.third_party_camera_frames[0]
        return frame[:, :, :3]

    def topdown_camera_hr(self, cam_idx: int = -1):
        frames = self.controller.last_event.third_party_camera_frames
    ...
    def manipulation_depth_frame(self):
        frame_hr = self.controller.last_event.third_party_depth_frames[0]  # HxW
        frame = cv2.resize(frame_hr, (396, 224), interpolation=cv2.INTER_NEAREST)
        cutoff = 6
        return frame[:, cutoff:-cutoff]
    
    @property
    def navigation_depth_frame(self):
        frame_hr = self.controller.last_event.depth_frame  # HxW
        frame = cv2.resize(frame_hr, (396, 224), interpolation=cv2.INTER_NEAREST)
        cutoff = 6
        return frame[:, cutoff:-cutoff]
    ...
    def get_segmentation_mask_of_object(
        ...
        if object_id in segmentation_to_look_at:
            mask_hr = segmentation_to_look_at[object_id].astype(np.uint8) * 255  # HxW bool -> uint8
            mask = cv2.resize(mask_hr, (396, 224), interpolation=cv2.INTER_NEAREST) > 0
            cutoff = 6
    ...
    def get_top_down_path_view(self, agent_path, targets_to_highlight=None):
        ...
        path = event.third_party_camera_frames[-1][:, :, :3]
        return path
